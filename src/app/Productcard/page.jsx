    ...
    <>
      <div className="group relative bg-white border border-slate-100 rounded-3xl transition-all duration-500 shadow-2xl flex flex-col overflow-hidden max-w-[250px] h-[350px] sm:h-auto sm:max-w-full mx-auto">
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300">
          <button onClick={triggerEdit} className="p-3 bg-white/90 backdrop-blur rounded-2xl text-slate-700 hover:bg-black hover:text-white shadow-xl transition-all">
            <Edit3 size={18} />
          </button>
          <button onClick={triggerDelete} className="p-3 bg-white/90 backdrop-blur rounded-2xl text-red-500 hover:bg-red-500 hover:text-white shadow-xl transition-all">
            <Trash2 size={18} />
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in zoom-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl relative">
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
              <input className="w-full p-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 ring-green-900/20 text-black" value={editForm.name} onChange={(e) => setEditForm({...editForm, name: e.target.value})} placeholder="Name" required />
              <input type="number" className="w-full p-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 ring-green-900/20 text-black" value={editForm.price} onChange={(e) => setEditForm({...editForm, price: e.target.value})} placeholder="Price" required />
              <button disabled={isProcessing} className="w-full py-5 bg-green-900 text-white rounded-2xl font-bold uppercase tracking-widest hover:bg-black transition-all">
      ...
      {showDeleteModal && (
       <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in zoom-in duration-300">
         <div className="bg-white w-full max-w-[90%] sm:max-w-sm rounded-[2.5rem] p-6 sm:p-10 text-center shadow-2xl">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Confirm Delete</h2>
            <p className="text-slate-500 text-sm mb-8">Remove <span className="font-bold">{product.name}</span> permanently?</p>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in zoom-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl">
            <div className="h-48 bg-slate-50 relative flex items-center justify-center">
              <Image src={imgSrc} fill className="object-contain p-6" alt="buy" />
              <button onClick={() => setShowBuyModal(false)} className="absolute top-4 right-4 p-2 bg-white rounded-full text-black"><X size={20}/></button>
            </div>
            <div className="p-10 text-center">
              <h2 className="text-2xl font-bold mb-2 text-black">Confirm Order</h2>
              <button onClick={() => sendWhatsAppMessage(product)} className="w-full py-5 bg-green-900 text-white rounded-2xl font-bold uppercase tracking-widest hover:bg-black transition-all">WhatsApp Order</button>
            </div>
          </div>
        </div>
      )}
